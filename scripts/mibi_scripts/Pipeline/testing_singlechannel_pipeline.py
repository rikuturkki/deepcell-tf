...
#    edge = np.copy(predictions[:,:,:,0])
    edge_og = skimage.io.imread('/deepcell-tf/scripts/feature_0_frame_0.tif')

    edge = np.copy(edge_og)
...
#    interior = np.copy(predictions[:, :, :, 1])
    ...
    interior_og = skimage.io.imread('/deepcell-tf/scripts/feature_1_frame_0.tif')
    interior = np.copy(interior_og)
    ...
    tiff.imsave(os.path.join(output_location, 'edge_prediction.tif'), edge_og)
    tiff.imsave(os.path.join(output_location, 'interior_bound.tif'), interior_og)
