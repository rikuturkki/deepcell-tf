...
class FlySamplingArrayIterator(Iterator):
    def __init__(self,
                 train_dict,
...
#         print(common_label, n_samples, self.y.shape)
        # Upsample each class
        new_b = []
        ...
        for label in range(n_labels+1):
            index = np.argwhere(self.y==label)
        ...
        new_b = np.array(new_b, dtype=np.int32)

        new_px = np.array(new_px, dtype=np.int32)
        new_py = np.array(new_py, dtype=np.int32)
        new_y = np.array(new_y, dtype=np.int32)

        shuffled_index = np.arange(len(new_b), dtype=np.int32)
    ...
    def _sample_image(self, b, px, py):
        ...
        if CHANNELS_FIRST:
            X_sample = self.x[b, :, px - win_x:px + win_x + 1, py - win_y:py + win_y + 1]
        else:
            X_sample = self.x[b, px - win_x:px + win_x + 1, py - win_y:py + win_y + 1, :]
...
class FlySamplingDataGenerator(ImageDataGenerator):
    def flow(self,
             train_dict,
             batch_size=32,
             shuffle=True,
             ...
             save_prefix='',
             save_format='png'):
        return FlySamplingArrayIterator(
            train_dict,
            self,
